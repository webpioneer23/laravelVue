  ...
  <div id="user-edit-tab-info">
    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 w-full">
        <vs-input class="w-full mt-4" label="Username" v-model="data_local.name" v-validate="'required|alpha_num'" name="name" />
        <span class="text-danger text-sm"  v-show="errors.has('name')">{{ errors.first('name') }}</span>

        <vs-input class="w-full mt-4" label="Country" v-model="data_local.country" v-validate="'required|alpha_spaces'" name="country" />
...
export default {
  ...
  methods: {
    ...
    save_changes () {
      ...
      const owner_data=this.data_local;
      axios.put(`/api/users/${owner_data.id}`,owner_data)
        .then((response) => {
      ...
      console.log("local_data",this.data_local);
